...
def set_user_preferences():
    try:
        ...
        if data['age_group'] not in valid_age_groups:
            return jsonify({'success': False, 'error': 'Invalid age group'}), 400
        if data['dietary_preference'] not in valid_dietary_preferences:
            return jsonify({'success': False, 'error': 'Invalid dietary preference'}), 400
        if data['fitness_goal'] not in valid_fitness_goals:
            return jsonify({'success': False, 'error': 'Invalid fitness goal'}), 400
...
def get_available_meals():
    try:
        ...
        # Get filtered meals based on preferences
...
def get_user_challenges():
    try:
        ...
        # Convert ObjectId to string and add challenge details
        for user_challenge in user_challenges:
            ...
            # Get challenge details from static list
...
def complete_challenge_day(user_challenge_id):
    try:
        ...
        if result.modified_count == 0:
            return jsonify({'success': False, 'error': 'User challenge not found'}), 404
        ...
        if user_challenge:
            ...
            if all_completed:
                ...
                # Award badge and experience based on challenge points
